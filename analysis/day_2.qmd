...
```{r}
read_strategy_guide <- function(.path) {
    strategy_guide <-
        .path %>%
        read_file() %>%
        str_split_1("\r\n") %>%
        ...
        transpose() %>%
        map(unlist) %>%
        set_names(c("elf", "player")) %>%
        as_tibble()
}

calculate_score_1 <- function(x) {
    shapes <- list("Rock", "Paper", "Scissors")
    shape_scores <- c(Rock = 1L, Paper = 2L, Scissors = 3L)
    elf_key <- set_names(shapes, c("A", "B", "C"))
    player_key <- set_names(shapes, c("X", "Y", "Z"))
    win_conditions <-
        list(Rock = "Paper", Paper = "Scissors", Scissors = "Rock")
    
    score <-
        x %>%
        mutate(
            elf_shape = map_chr(elf, .f = ~elf_key[[.x]]),
            player_shape = map_chr(player, .f = ~player_key[[.x]]),
            shape_score =
                map_int(player_shape, .f = ~shape_scores[[.x]]),
            outcome = map2_int(
                elf_shape, player_shape, .f = ~{
                    if (.x == .y)
                        return(3)
                    if (win_conditions[[.x]] == .y)
                        return(6)
                    return(0)
                }
            ),
...
```{r}
#| message: FALSE
example_data <- read_strategy_guide(here("data", "example_2.txt"))
example_score_1 <- calculate_score_1(example_data)
example_score_1
```
...
#| include: FALSE
#| message: FALSE
input_data <- read_strategy_guide(here("data", "input_2.txt"))
score_1 <- calculate_score_1(input_data)
score_1
```
...
```{r}
calculate_score_2 <- function(x) {
    shapes <- list("Rock", "Paper", "Scissors")
    shape_scores <- c(Rock = 1L, Paper = 2L, Scissors = 3L)
    elf_key <- set_names(shapes, c("A", "B", "C"))
    player_key <- list(X = 0L, Y = 3L, Z = 6L)
    win_conditions <-
        list(Rock = "Paper", Paper = "Scissors", Scissors = "Rock")
    loose_conditions <-
        list(Rock = "Scissors", Paper = "Rock", Scissors = "Paper")
    
    score <-
        x %>%
        mutate(
            elf_shape = map_chr(elf, .f = ~elf_key[[.x]]),
            outcome = map_int(player, .f = ~player_key[[.x]]),
            player_shape = map2_chr(
                outcome, elf_shape, .f = ~{
                    if (.x == 3)  # Draw
                        return(.y)
                    if(.x == 6)  # Win
                        return(win_conditions[[.y]])
                    if (.x == 0)
                        return(loose_conditions[[.y]])
                }
            ),
            shape_score =
                map_int(player_shape, .f = ~shape_scores[[.x]]),
...
```{r}
example_score <- calculate_score_2(example_data)
example_score
```

```{r}
#| include: FALSE
score_2 <- calculate_score_2(input_data)
score_2
```
