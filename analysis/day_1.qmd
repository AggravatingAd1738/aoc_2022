...
```{r}
collect_calories <- function(.path) {
    elf_calories <-
        .path %>%
        read_file() %>%
        str_split_1("\r\n\r\n") %>%
        str_split("\r\n") %>%
        map(.f =  ~as.double(.x) %>% discard(is.na)) %>%
        as_tibble_col(column_name = "calories") %>%
        mutate(elf = row_number()) %>%
        unnest(calories)
}
calories_top_elfs <- function(x, n = 1) {
    calories <-
        x %>%
        group_by(elf) %>%
        summarise(total_calories = sum(calories)) %>%
        slice_max(total_calories, n = n) %>%
        pull(total_calories) %>%
        sum()
}
```
...
```{r}
example_data <- collect_calories(here("data", "example_1.txt"))
example_top_elf <- calories_top_elfs(example_data)
...
```{r}
#| include: FALSE
input_data <- collect_calories(here("data", "input_1.txt"))
top_elf <- calories_top_elfs(input_data)
...
already realized that the Elf carrying the most Calories of food
...
```{r}
example_top_three <- calories_top_elfs(example_data, n = 3)
example_top_three
```

```{r}
#| include: FALSE
top_three_elfs <- calories_top_elfs(input_data, n = 3)
```
